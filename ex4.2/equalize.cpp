...
int main(int argc, char** argv){
  ...
  while(1){
    ...
    histImgR.copyTo(realce(cv::Rect(0, 0       ,nbins, histh)));
    histImgG.copyTo(realce(cv::Rect(0, histh   ,nbins, histh)));
    histImgB.copyTo(realce(cv::Rect(0, 2*histh ,nbins, histh)));

    
    
    cv::imshow("Ao vivo", realce);
    key = cv::waitKey(30);
    if(key == 27) break;
  }
  return 0;
